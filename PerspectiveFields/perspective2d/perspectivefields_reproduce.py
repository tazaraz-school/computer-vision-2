...
class PerspectiveFields(nn.Module):
    ...
    def _init_weights(self):
        ...
        if self.version in model_zoo:
            ...
            #self.load_state_dict(state_dict, strict=False) edited
    ...
    def inference_batch(self, img_bgr_list):
        ...
        for img_bgr in img_bgr_list:
            ...
            if self.input_format == "RGB":
                ...
                original_image = original_image[:, :, ::-1]
            height, width = original_image.shape[:2]
            image = self.aug.apply_image(original_image)           #  reproduce check this out later 
            image = torch.as_tensor(image.astype("float32").transpose(2, 0, 1))
            inputs = {"image": image, "height": height, "width": width}
            input_list.append(inputs)

        images = [x["image"].to(self.device) for x in input_list] # edit 
        images = [(x - self.pixel_mean) / self.pixel_std for x in images] #
        images = torch.stack(images)#

        predictions = self.forward(images)
        
        return images,predictions # also return l'images edit
    ...
    def forward(self, batched_inputs) -> dict:
        ...
        images = batched_inputs.clone() # need tensor input for grad cam, but this code relies on batched_input dicts as well
        batched_inputs = [
            {
                "image": img,                # shape: [3, 256, 256]
                "height": 256,      # 256
                "width": 256        # 256 # TODOOOOOOOO: MAKE DYNAMIC ORIGINAL H W AS WE AUGMENT size using self.AUG
            }
