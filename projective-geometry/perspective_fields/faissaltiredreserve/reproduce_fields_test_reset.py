...
from reproduce_fields_dataset_reset import get_test_dataloaders
from reproduce_fields_model_reset import load_model
import cv2
...
def test(model, test_dataloader, save_path, test_type):
    ...
    for images, labels in tqdm(test_dataloader, desc="testing"):
        with torch.no_grad(): # edit: put it inside loop to allow cam grad to use grad
            ...
            #from PIL import Image as ru

            #print("TORCH TENEEEEEE\n",T.PILToTensor()(ru.open(images[0]))[1][223][123])
            labels = labels.to(device)

            outputs = model.forward([cv2.imread(imagepath) for imagepath in images]) # edited (was just images as input)  + "images" output for gradcam
