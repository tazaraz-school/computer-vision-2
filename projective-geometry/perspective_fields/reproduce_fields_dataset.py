...
def get_test_dataloaders(test_image_paths, unconfident_image_paths, misclassified_image_paths, class_to_idx):  
    ...
    #easy_image_paths = sample(easy_image_paths,1) #int(len(easy_image_paths)*0.1)) #eeeee
    #unconfident_image_paths = sample(unconfident_image_paths,1) #int(len(easy_image_paths)*0.1)) #eeeee
    #misclassified_image_paths = sample(misclassified_image_paths,1) #int(len(easy_image_paths)*0.1)) #eeeee
    ...
    unconfident_dataloader = DataLoader(unconfident_dataset, batch_size=bs, shuffle=False)
    
    misclassified_dataset = PerspectiveMapDataset(misclassified_image_paths, class_to_idx)
    misclassified_dataloader = DataLoader(misclassified_dataset, batch_size=bs, shuffle=False)
